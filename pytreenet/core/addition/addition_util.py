...
def generate_tree_pair(depth: int, branching_factor: int = 2, leaf_probability: float = 0.2,
    ...
    """
    Generates two structurally identical TTNs with different random tensor values.

    Uses probabilistic early termination to create irregular tree structures.

    Args:
        depth: Maximum tree depth (e.g., 0 = root only, 1 = root + children)
        branching_factor: Children per internal node. Defaults to 2.
        leaf_probability: Probability of early leaf termination. Defaults to 0.2.
        physical_dim: Physical index dimension. Defaults to 2.
        bond_dim: Virtual index dimension. Defaults to 2.

    Returns:
        Two structurally identical TTNs with different tensors
    """
...
def generate_tree_pair_by_node_count(num_nodes: int,
    ...
    Builds trees level-by-level with the following structure: 0(1(3,4), 2(5,6), ...)

    Args:
        num_nodes: Target number of nodes
        physical_dim: Physical dimension per node. Defaults to 2.
        bond_dim: Virtual bond dimension. Defaults to 2.
...
def tree_statistics(ttn: TreeTensorNetwork) -> dict:
    ...
    Returns:
        dict: Statistics including node count, depth and parameters.
    """
    if not ttn.nodes:
